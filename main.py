...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
                ...
                if event.content and event.content.parts:
                    for part in event.content.parts:
                        if part.inline_data and part.inline_data.data:
                            ...
                            # Tool responses are internal — image is in slide_queue
                            pass
                        elif part.text:
    ...
    async def slide_drainer_task():
        """Dedicated slide delivery — polls slide_queue independently.

        Runs every 300ms so slides arrive promptly even when the event
        stream is paused during tool execution.
        """
        try:
            while True:
                while not image_tool.slide_queue.empty():
                    ...
                    try:
                        ...
                        logger.info(
                            "Slide delivered to client: '%s' (%d KB)",
                            slide_data["topic"],
                            len(slide_data["image"]) // 1024,
                        )
                    except Exception as e:
                        logger.error("Failed to send slide '%s': %s", slide_data.get("topic"), e)
                await asyncio.sleep(0.3)
        except WebSocketDisconnect:
            logger.info("Client disconnected (slide drainer): %s", session_id)
